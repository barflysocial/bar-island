...
function publicSession(session, viewerId=null) {
  const couples = session.couples.map(c => ({
    ...c,
    players: c.players.map(pid => session.players[pid]).filter(Boolean).map(p => ({ id:p.id, name:p.name, bio:p.bio, greenFlag:p.greenFlag, redFlag:p.redFlag, type:p.type, pickup:p.pickup, status:p.status }))
  }));
  const players = Object.values(session.players).map(p => ({
    id:p.id, name:p.name, bio:p.bio, greenFlag:p.greenFlag, redFlag:p.redFlag, type:p.type, pickup:p.pickup, status:p.status, coupleId:p.coupleId || null, isViewer: p.id === viewerId
  }));
...
app.get('/api/sessions', (req, res) => {
  const db = load();
  const sessions = Object.values(db.sessions).sort((a,b)=>{
    const at = a.scheduledAt || a.createdAt;
    const bt = b.scheduledAt || b.createdAt;
    return at.localeCompare(bt);
  }).map(s => publicSession(s));
  res.json({ serverTime: nowIso(), sessions });
});
app.post('/api/sessions', hostOnly, (req, res) => {
  const db = load();
...
app.post('/api/host/:id/phase', hostOnly, (req, res) => {
...
app.post('/api/host/:id/pair', hostOnly, (req, res) => {
  const db = load(); const session = db.sessions[req.params.id];
  if (!session) return res.status(404).json({ error: 'Session not found.' });
  pairPlayers(session); session.phase = 'firstCoupling'; session.status = 'live'; session.round = 1; session.updatedAt = nowIso();
  save(db); res.json({ session: publicSession(session) });
});
app.post('/api/host/:id/dump', hostOnly, (req, res) => {
  const db = load(); const session = db.sessions[req.params.id];
  if (!session) return res.status(404).json({ error: 'Session not found.' });
...
app.post('/api/host/:id/delete', hostOnly, (req, res) => {
  const db = load();
