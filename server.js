...
function createSession(body){
  ...
  return {
    id:id('session'), code: code(), title: body.title || 'Bar Island', venue: body.venue || '', voteIframeUrl: body.voteIframeUrl || '', requirePlayerPhoto: body.requirePlayerPhoto !== false, startTime,
    ...
    players:[], audience:[], couples:[], votes:[], answers:[], recouplingChoices:[], finalePitches:[], cashoutChoices:[], reactions:[], predictions:[], audienceQuestions:[],
...
app.get('/api/sessions', async (req,res)=>{
  ...
  res.json({ storage: pool?'postgres':'memory', dbDiagnostic: dbDiagnostic(), serverNow:nowIso(), hostState, activeSession: active, sessions: sessions.map(publicSession) });
});
