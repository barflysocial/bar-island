...
const PHASE_CATS = { challenge1:['compatibility','redflag'], challenge2:['spill','audience'], vote:['audience','redflag'], finale:['finale'] };
const PHASE_FALLBACK = { challenge1:'C001', challenge2:'S041', vote:'A081', finale:'F091' };
...
function createQuestionSet(session, mode='fresh'){
  ...
  const used = new Set();
  const set=[];
  for(const phase of ['challenge1','challenge2','vote','finale']){
...
function isQuestionPhase(phase){ return ['challenge1','challenge2','vote','finale'].includes(phase); }
...
function normalizeAnswerForCouple(answer, playerId, couple){
  ...
  if(txt === 'my partner') return playerId === a ? b : a;
  return txt;
}
...
function currentPrompt(session){ const q=getCurrentQuestion(session); if(q) return q.text; const prompts={checkin:'Create a funny islander profile and wait for the host to start.',firstCoupling:'First Impressions: couples are forming. Make your best first impression.',mingle:'Couples, start mingling. Challenges begin after the talk timer.',recoupling:'Recoupling: stay loyal, switch, or become single. No couple is fully safe.',ended:'Game ended. Congratulations to the Bar Island Champions.'}; return prompts[session.phase] || 'Wait for the host to start the next Bar Island moment.'; }
...
function advancePhase(session,phase){ session.phase=phase; session.status=phase==='ended'?'completed':'live'; const roundMap={checkin:0,firstCoupling:1,mingle:1,challenge1:2,challenge2:3,vote:4,recoupling:4,finale:5,ended:5}; session.round=roundMap[phase] ?? session.round; session.votes={}; if(isQuestionPhase(phase)){ startQuestionTimer(session,phase,true); } else { session.currentQuestionId=''; session.questionStartedAt=''; session.questionEndsAt=''; session.questionLocked=false; } if(phase==='mingle') startMingleTimer(session,true); if(phase!=='mingle'){ session.mingleStartedAt=session.mingleStartedAt||''; } if(phase==='recoupling') session.recouplingRequests={}; }
...
app.get('/api/questions',(req,res)=>res.json({count:QUESTION_POOL.length,categories:{compatibility:40,spill:25,redflag:15,audience:10,finale:10},questions:QUESTION_POOL.map(q=>({id:q.id,cat:q.cat,type:q.type,seconds:q.seconds,text:q.text}))}));
