...
app.post('/api/host/delete-completed', hostOnly, asyncHandler(async(req,res)=>{ const activeState=await getHostState('activeSessionId'); if(!usePostgres){ for(const [sid,s] of Object.entries(memoryDb.sessions)){ if(isCompletedSession(s)) delete memoryDb.sessions[sid]; } if(activeState?.id && !memoryDb.sessions[activeState.id]) await setHostState('activeSessionId',{id:'',updatedAt:nowIso(),reason:'delete_completed'}); return res.json({ok:true}); } const r=await pool.query("DELETE FROM sessions WHERE status='completed' OR (data->>'phase')='ended' OR (data->>'status')='completed' OR (data ? 'clearedAt')"); if(activeState?.id){ const stillThere=await getSessionById(activeState.id); if(!stillThere || isCompletedSession(stillThere)) await setHostState('activeSessionId',{id:'',updatedAt:nowIso(),reason:'delete_completed'}); } res.json({ok:true,deleted:r.rowCount}); }));
