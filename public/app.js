...
function audiencePhasePanel(s){
  if(s.phase==='ended' || s.status==='completed') return winnerPanel(s);
  if(['redFlag','chemistry','spillTea','temptation','survival','finale','winnerVote'].includes(s.phase)) return votePanel(s);
...
function votePanel(s){ const active=['redFlag','chemistry','spillTea','temptation','survival','finale','winnerVote'].includes(s.phase); const m=me(); return `<div class="card"><h1>Vote</h1><p>${state.role==='player'?'Players can vote for other couples only.':'Audience voters can vote for any couple.'}</p>${s.couples.map(c=>{ const own=state.role==='player' && m?.coupleId===c.id; return `<div class="vote-row profile"><div><div class="team-photos small-team">${c.players.map(p=>playerPhoto(p)).join('')}</div><b>${esc(c.players.map(p=>p.name).join(' + ')||'Single')}</b><br><span class="muted">${own?'Your couple — self vote blocked':'Eligible for vote'}</span></div><button ${(!active||own)?'disabled':''} onclick="vote('${c.id}')">Vote</button></div>`; }).join('')}</div>`; }
