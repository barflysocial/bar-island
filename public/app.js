...
function clearPlayerSessionLocal(){
  ['barIslandCode','barIslandPlayerId','barIslandAudienceId','barIslandRole','barIslandPublicCode','barIslandActiveGameId'].forEach(k=>localStorage.removeItem(k));
  state.code=''; state.playerId=''; state.audienceId=''; state.role='player'; state.session=null;
}
function sessionEndedScreen(message='This game session has ended or was cleared.'){
  clearInterval(state.timer);
  clearPlayerSessionLocal();
  app.innerHTML=`<section class="screen"><div class="card guided-card"><h1>Session Ended</h1><p>${esc(message)}</p><button onclick="landing()">Return Home</button></div></section>`;
}

async function api(url, opts={}){ const res=await fetch(url,{headers:{'Content-Type':'application/json'},...opts}); const data=await res.json().catch(()=>({})); if(!res.ok){ const err=new Error(data.error || 'Request failed.'); err.status=res.status; err.data=data; throw err; } return data; }
...
function phaseLabel(s){
  const labels={checkin:'Check-In / Profile',firstImpressions:'First Impressions',firstCoupling:'Couple Up',mingle:'Mingle Time',redFlag:'Red Flag / Green Flag',chemistry:'Chemistry Check',spillTea:'Spill the Tea',temptation:'Temptation',recoupling:'Recoupling',survival:'Survival Challenge',finale:'Finale Pitch',winnerVote:'Final Winner Vote',cashout:'Cash Out or Double Down',ended:'Winner Reveal'};
  return labels[s.phase] || 'Bar Island';
}
function nextStep(s){
  const next={checkin:'Next: first impressions',firstImpressions:'Next: couple up',firstCoupling:'Next: mingle time',mingle:'Next: Red Flag / Green Flag',redFlag:'Next: Chemistry Check',chemistry:'Next: Spill the Tea',spillTea:'Next: Temptation / Switch-Up',temptation:'Next: Recoupling',recoupling:'Next: Survival Challenges',survival:'Next: survival vote or dumping',finale:'Next: Final Winner Vote',winnerVote:'Next: winner reveal',cashout:'Next: replacement couples or the next session',ended:'Game complete'};
...
function playerCue(s){
  const cues={
    checkin:{now:'Lobby Countdown',do:'You are checked in. Wait for the scheduled start time and keep your profile ready.',next:'First Impressions'},
    firstImpressions:{now:'First Impressions',do:'Watch the public screen while the host introduces the islanders.',next:'Couple Up'},
    firstCoupling:{now:'Couple Reveal',do:'Find your partner and get ready to mingle.',next:'Mingle Time'},
    mingle:{now:'Mingle Time',do:'Talk with your partner. Optional starters are here only if you need them.',next:'Red Flag / Green Flag'},
    redFlag:{now:'Red Flag / Green Flag',do:'Answer quickly and honestly. Your partner is answering too.',next:'Chemistry Check'},
    chemistry:{now:'Chemistry Check',do:'Pick the answer that best fits you or your partner. Matching energy matters.',next:'Spill the Tea'},
    spillTea:{now:'Spill the Tea',do:'Give the crowd something fun. Keep it playful and bar-friendly.',next:'Temptation / Switch-Up'},
    temptation:{now:'Temptation / Switch-Up',do:'Answer carefully. This round can shake up the island.',next:'Recoupling'},
    recoupling:{now:'Recoupling',do:'Choose whether to stay loyal, switch, steal, or go single.',next:'Survival Challenges'},
    survival:{now:'Survival Challenge',do:'Every answer and vote matters. Couples can be dumped from the island.',next:'Vote or Dumping Ceremony'},
    vote:{now:'Audience Vote',do:'Cast your vote once. Your selected card turns green when it counts.',next:'Results Reveal'},
    winnerVote:{now:'Final Winner Vote',do:'Vote for the couple you want crowned Bar Island Champions.',next:'Winner Reveal'},
    finale:{now:'Finale Pitch',do:'Submit your final pitch. Tell the crowd why your couple should win.',next:'Final Winner Vote'},
    cashout:{now:'Cash Out or Double Down',do:'Choose wisely. Only one partner has to cash out for the couple to leave.',next:'Next Session or Replacement Couples'},
    ended:{now:'Winner Reveal',do:'The game is complete. Watch the public screen for the final reveal.',next:'Return Home'}
  };
  const c=cues[s.phase]||{now:phaseLabel(s),do:s.currentPrompt||'Wait for the host to continue.',next:'Next phase'};
  return `<div class="cue-card player-cue"><div class="cue-eyebrow">What’s happening now</div><h2>${esc(c.now)}</h2><p>${esc(c.do)}</p><div class="next-step">Next: ${esc(c.next)}</div></div>`;
}
...
function renderGame(){
  ...
  if(state.role==='player') app.innerHTML = `<section class="screen player-flow">${statusStrip(s)}${playerCue(s)}${playerPhasePanel(s)}</section>`;
  else app.innerHTML = `<section class="screen player-flow">${statusStrip(s)}${playerCue(s)}${audiencePhasePanel(s)}</section>`;
  refreshClockBadges();
}
function playerPhasePanel(s){
  ...
  if(s.cashout?.active) return cashoutPanel(s);
  if(s.phase==='checkin') return lobbyPanel(s);
  ...
  if(q && ['redFlag','chemistry','spillTea','temptation','survival','finale'].includes(s.phase)) return challengePanel(s);
  if(['redFlag','chemistry','spillTea','temptation','survival','finale'].includes(s.phase)) return `<div class="card guided-card"><h1>Waiting for the Question</h1><p>The host is about to start the next prompt.</p><div class="next-step">${nextStep(s)}</div></div>`;
...
function audiencePhasePanel(s){
  if(s.phase==='ended' || s.status==='completed') return winnerPanel(s);
  if(s.phase==='checkin') return audienceLobbyPanel(s);
  if(['redFlag','chemistry','spillTea','temptation','survival','finale','winnerVote','vote'].includes(s.phase)) return votePanel(s);
...
function myAnswer(){
  ...
  if(s.viewerFinalePitch && (s.viewerFinalePitch.questionId === q.id || s.phase === 'finale')) return s.viewerFinalePitch;
  return (s.challengeAnswers || []).find(a => a.questionId === q.id && a.playerId === state.playerId) || (s.finalePitches || []).find(a => a.questionId === q.id && a.playerId === state.playerId) || null;
}
...
function partnerName(){ const c=myCouple(); const partner=c?.players?.find(p=>p.id!==state.playerId); return partner?.name || 'My Partner'; }
function coupleNames(){ const c=myCouple(); return c?.players?.map(p=>p.name).join(' + ') || 'Your Couple'; }
function questionText(q){ const m=me(); if(!q) return ''; const name=m?.name||'Islander'; if(q.type==='typed') return `${coupleNames()}, ${q.text}`; return `${name}, ${q.text}`; }
function optionLabel(label=''){
  const val=optionValue(label);
  if(val==='me') return 'Me';
  if(val==='partner') return partnerName();
  if(val==='both') return 'Both';
  if(val==='neither') return 'Neither';
  return label;
}
function draftKey(qid){ return `barIslandDraft_${state.code}_${state.playerId}_${qid}`; }
function getAnswerDraft(qid){ return localStorage.getItem(draftKey(qid)) || ''; }
function saveAnswerDraft(qid, value){ if(!qid) return; localStorage.setItem(draftKey(qid), value || ''); }
function clearAnswerDraft(qid){ if(qid) localStorage.removeItem(draftKey(qid)); }
function recoupleDraftKey(){ return `barIslandRecouple_${state.code}_${state.playerId}_${state.session?.currentRecoupling?.startedAt||'active'}`; }
function getRecoupleDraft(){ try{return JSON.parse(localStorage.getItem(recoupleDraftKey())||'null');}catch{return null;} }
function saveRecoupleDraft(choice){ if(!state.code||!state.playerId) return; localStorage.setItem(recoupleDraftKey(), JSON.stringify(choice||{})); }
function clearOldRecoupleDrafts(){ const prefix=`barIslandRecouple_${state.code}_${state.playerId}_`; Object.keys(localStorage).forEach(k=>{ if(k.startsWith(prefix) && k!==recoupleDraftKey()) localStorage.removeItem(k); }); }
function lobbyPanel(s){
  const m=me(); const starts=s.lobby?.startsAt || s.scheduledAt || ''; const ready=s.lobby?.ready || (starts && Date.parse(starts)<=serverNowMs());
  return `<div class="card guided-card lobby-card"><h1>Bar Island Lobby</h1><div class="center">${playerPhoto(m,'avatar large')}</div><p>You're checked in as <b>${esc(m?.name||'Islander')}</b>.</p>${starts?`<div class="mega-count">${ready?'Ready to Start':`<span data-countdown-target="${esc(starts)}">${countdownText(starts)}</span>`}</div><p class="muted">Scheduled start: ${esc(new Date(starts).toLocaleString())}</p>`:`<p class="leader">Waiting for the host to set the game time.</p>`}<div class="grid two"><div class="profile"><b>Players Checked In</b><span>${s.players.length}/${s.maxPlayers}</span></div><div class="profile"><b>Audience</b><span>${s.audienceCount||0} watching/voting</span></div></div><p class="muted">The game will begin when the host starts from the scheduled game time.</p><div class="next-step">Next: First Impressions</div></div>`;
}
function audienceLobbyPanel(s){ const starts=s.lobby?.startsAt || s.scheduledAt || ''; const ready=s.lobby?.ready || (starts && Date.parse(starts)<=serverNowMs()); return `<div class="card guided-card lobby-card"><h1>Audience Lobby</h1>${starts?`<div class="mega-count">${ready?'Ready to Start':`<span data-countdown-target="${esc(starts)}">${countdownText(starts)}</span>`}</div><p class="muted">Scheduled start: ${esc(new Date(starts).toLocaleString())}</p>`:''}<p>You are checked in for audience voting. Voting opens when the host starts a voting moment.</p><div class="grid two"><div class="profile"><b>Players</b><span>${s.players.length}/${s.maxPlayers}</span></div><div class="profile"><b>Audience</b><span>${s.audienceCount||0}</span></div></div></div><div class="card"><h2>Current Islanders</h2>${s.players.length?s.players.map(p=>profile(p)).join(''):'<p>Waiting on players to join.</p>'}</div>`; }
...
function cashoutPanel(s){
  const c=s.cashout||{};
  if(state.role!=='player') return `<div class="card guided-card"><h1>Cash Out or Double Down</h1><p>The top couples are deciding whether to take the revealed prize or stay on the island.</p></div>`;
  const m=me(); const myC=myCouple();
  if(!myC) return `<div class="card guided-card"><h1>Cash Out or Double Down</h1><p>You are not in an active couple for this cashout decision.</p></div>`;
  const decision=c.viewerDecision;
  const prize=c.prize||'Prize to be announced';
  if(decision) return `<div class="card guided-card"><h1>Choice Submitted</h1><div class="profile ${decision.choice==='cashout'?'danger':'good'}"><b>${decision.choice==='cashout'?'Cash Out':'Double Down'}</b><span>Session ${esc(c.sessionNumber||'')}</span><span>Cash Out Prize: ${esc(prize)}</span><span class="muted">If either partner chose Cash Out, the couple cashes out.</span></div><p>Waiting for the host to lock choices.</p></div>`;
  return `<div class="card guided-card"><h1>Cash Out or Double Down</h1><p>Your couple survived Session ${esc(c.sessionNumber||'')}.</p><div class="profile"><b>Revealed Cash Out Prize</b><span>${esc(prize)}</span></div><p>Only one partner has to choose Cash Out for the whole couple to leave. If both stay, you double down for the bigger mystery final prize.</p><form id="cashoutForm" class="grid one"><button class="danger" type="submit" name="choice" value="cashout">Cash Out</button><button class="teal" type="submit" name="choice" value="doubleDown">Double Down</button></form></div>`;
}

function winnerPanel(s){
  const winner=s.winner?.coupleName || s.voteCounts?.[0]?.coupleName || 'Bar Island Champions';
  const finalPrize=s.cashout?.finalPrize||''; return `<div class="card guided-card winner-card"><h1>Winner Reveal</h1><div class="winner-name">${esc(winner)}</div>${finalPrize?`<div class="profile good"><b>Final Grand Prize</b><span>${esc(finalPrize)}</span></div>`:'<p class="muted">Final Grand Prize: Mystery Prize</p>'}<p>Thanks for playing Bar Island.</p></div>`;
}
...
function challengePanel(s){
  ...
  return `<div class="card"><div class="row between"><h1>${s.phase==='finale'?'Finale Pitch':'Question'}</h1><span class="pill timer-pill">Time Left <span data-countdown-target="${esc(q.endsAt)}">${countdownText(q.endsAt)}</span></span></div><p class="leader">${esc(questionText(q))}</p><p>Your couple: <b>${esc(c.players.map(p=>p.name).join(' + '))}</b></p>${ans?`<div class="profile good"><b>${s.phase==='finale'?'Finale Pitch Submitted':'Submitted'}</b><span>${esc(ans.text || ans.pitch || ans.option)}</span><span class="muted">Your answer is saved. Refreshing your phone will not clear it.</span></div>`:answerForm(q,locked)}</div>`;
}
function answerForm(q, locked){ if(q.type==='choice'){ return `<form id="answerForm" class="grid one">${(q.options||[]).map(o=>`<button type="submit" name="option" value="${esc(optionValue(o))}" data-label="${esc(optionLabel(o))}" ${locked?'disabled':''}>${esc(optionLabel(o))}</button>`).join('')}</form>${locked?'<p class="danger pill">Time is up.</p>':''}`; } const draft=getAnswerDraft(q.id); const isFinale=state.session?.phase==='finale'; return `<form id="answerForm"><label>${isFinale?'Finale Pitch':'Your Answer'}<textarea name="text" ${locked?'disabled':''} maxlength="220" placeholder="${isFinale?'Tell the crowd why your couple should win':'Keep it funny and bar-friendly'}" oninput="saveAnswerDraft('${esc(q.id)}', this.value)">${esc(draft)}</textarea></label><button ${locked?'disabled':''}>${isFinale?'Submit Finale Pitch':'Submit Answer'}</button></form>${locked?'<p class="danger pill">Time is up.</p>':''}`; }
function votePanel(s){ const voteState=s.vote||{}; const active=['redFlag','chemistry','spillTea','temptation','survival','finale','winnerVote','vote'].includes(s.phase) && (!voteState.active || !voteState.locked); const m=me(); const voted=s.viewerVote||null; const timer=voteState.active&&voteState.endsAt?`<span class="pill timer-pill">Voting ${voteState.locked?'Locked':'Open'} · <span data-countdown-target="${esc(voteState.endsAt)}">${countdownText(voteState.endsAt)}</span></span>`:'<span class="pill">Voting available when opened</span>'; return `<div class="card"><div class="row between"><h1>Vote</h1>${timer}</div><p>${state.role==='player'?'Players can vote for other couples only.':'Audience voters can vote for any couple.'}</p>${voted?'<p class="profile good"><b>Vote submitted.</b><span>You can vote again when the next voting round opens.</span></p>':''}${voteState.locked?'<p class="profile danger"><b>Voting closed.</b><span>Results are being counted.</span></p>':''}${s.couples.map(c=>{ const own=state.role==='player' && m?.coupleId===c.id; const selected=voted?.coupleId===c.id; const disabled=(!active||own||Boolean(voted)); return `<div class="vote-row profile ${selected?'voted-card good':(voted?'faded':'')}"><div><div class="team-photos small-team">${c.players.map(p=>playerPhoto(p)).join('')}</div><b>${esc(c.players.map(p=>p.name).join(' + ')||'Single')}</b><br><span class="muted">${selected?'Your vote was cast.':own?'Your couple — self vote blocked':voteState.locked?'Voting closed':voted?'Voting locked for this round':'Eligible for vote'}</span></div><button class="${selected?'teal':''}" ${disabled?'disabled':''} onclick="vote('${c.id}')">${selected?'Voted ✓':'Vote'}</button></div>`; }).join('')}</div>`; }
function recouplePanel(s){
  ...
  const rec=s.currentRecoupling||{};
  clearOldRecoupleDrafts();
  const submitted=rec.viewerChoice || getRecoupleDraft();
  const locked=Boolean(rec.locked || (rec.endsAt && Date.parse(rec.endsAt)<=serverNowMs()));
  const active=s.phase==='recoupling' && !locked;
  const otherPlayers=s.players.filter(p=>p.id!==state.playerId && !['dumped','cashedOut'].includes(p.status));
  const targetName=submitted?.targetPlayerId ? (s.players.find(p=>p.id===submitted.targetPlayerId)?.name||'Selected player') : 'No target';
  if(submitted){
    const pendingLocal = !rec.viewerChoice && submitted;
    return `<div class="card guided-card"><div class="row between"><h1>Recoupling Choice Submitted</h1>${rec.endsAt?`<span class="pill timer-pill">Time Left <span data-countdown-target="${esc(rec.endsAt)}">${countdownText(rec.endsAt)}</span></span>`:''}</div><div class="profile good"><b>Your move: ${esc(submitted.choice)}</b><span>Target: ${esc(targetName)}</span><span class="muted">${pendingLocal?'Saved on this phone and reconnecting to server...':'Your choice is saved. Refreshing your phone will not clear it.'}</span></div>${active?`<details><summary>Change my choice before time runs out</summary>${recoupleForm(otherPlayers, submitted)}</details>`:`<p class="muted">Waiting for the host to announce recoupling results.</p>`}</div>`;
  }
  return `<div class="card guided-card"><div class="row between"><h1>Recoupling</h1>${rec.endsAt?`<span class="pill timer-pill">Time Left <span data-countdown-target="${esc(rec.endsAt)}">${countdownText(rec.endsAt)}</span></span>`:''}</div><p>Choose your move. The host sees all requests and announces the result.</p>${recoupleForm(otherPlayers, null, !active)}${locked?'<p class="danger pill">Recoupling time is up.</p>':''}</div>`;
}
...
document.addEventListener('submit',async ev=>{
  if(ev.target.id==='answerForm'){
    ...
    try{ const qid=state.session?.currentQuestion?.id; const data=await api('/api/answer',{method:'POST',body:JSON.stringify({code:state.code,playerId:state.playerId,option,text})}); if(qid) clearAnswerDraft(qid); state.session=data.session; syncClock(data.session.serverTime); renderGame(); toast('Answer submitted.'); }catch(e){toast(e.message);}
  }
  if(ev.target.id==='cashoutForm'){
    ev.preventDefault(); const submitter=ev.submitter; const choice=submitter?.value||''; try{ const data=await api('/api/cashout',{method:'POST',body:JSON.stringify({code:state.code,playerId:state.playerId,choice})}); state.session=data.session; syncClock(data.session.serverTime); renderGame(); toast('Cashout choice submitted.'); }catch(e){toast(e.message);}
  }
  if(ev.target.id==='recoupleForm'){
    ev.preventDefault();
    const f=Object.fromEntries(new FormData(ev.target).entries());
    const localChoice={playerId:state.playerId,choice:f.choice||'stay',targetPlayerId:f.targetPlayerId||'',recouplingStartedAt:state.session?.currentRecoupling?.startedAt||'',updatedAt:new Date(serverNowMs()).toISOString()};
    saveRecoupleDraft(localChoice);
    try{ const data=await api('/api/recouple',{method:'POST',body:JSON.stringify({code:state.code,playerId:state.playerId,...f})}); state.session=data.session; syncClock(data.session.serverTime); const serverChoice=data.session?.currentRecoupling?.viewerChoice; if(serverChoice) saveRecoupleDraft(serverChoice); renderGame(); toast('Recoupling choice submitted.'); }catch(e){toast(e.message); renderGame();}
  }
});
