...
function answerForm(q, locked){ if(q.type==='choice'){ return `<form id="answerForm" class="grid one">${(q.options||[]).map(o=>`<button type="submit" name="option" value="${esc(optionValue(o))}" data-label="${esc(optionLabel(o))}" ${locked?'disabled':''}>${esc(optionLabel(o))}</button>`).join('')}</form>${locked?'<p class="danger pill">Time is up.</p>':''}`; } const draft=getAnswerDraft(q.id); return `<form id="answerForm"><label>Your Answer<textarea name="text" ${locked?'disabled':''} maxlength="220" placeholder="Keep it funny and bar-friendly" oninput="saveAnswerDraft('${esc(q.id)}', this.value)">${esc(draft)}</textarea></label><button ${locked?'disabled':''}>Submit Answer</button></form>${locked?'<p class="danger pill">Time is up.</p>':''}`; }
function votePanel(s){ const active=['redFlag','chemistry','spillTea','temptation','survival','finale','winnerVote','vote'].includes(s.phase); const m=me(); const voted=s.viewerVote||null; return `<div class="card"><h1>Vote</h1><p>${state.role==='player'?'Players can vote for other couples only.':'Audience voters can vote for any couple.'}</p>${voted?'<p class="profile good"><b>Vote submitted.</b><span>You can vote again when the next voting round opens.</span></p>':''}${s.couples.map(c=>{ const own=state.role==='player' && m?.coupleId===c.id; const selected=voted?.coupleId===c.id; const disabled=(!active||own||Boolean(voted)); return `<div class="vote-row profile ${selected?'voted-card good':(voted?'faded':'')}"><div><div class="team-photos small-team">${c.players.map(p=>playerPhoto(p)).join('')}</div><b>${esc(c.players.map(p=>p.name).join(' + ')||'Single')}</b><br><span class="muted">${selected?'Your vote was cast.':own?'Your couple — self vote blocked':voted?'Voting locked for this round':'Eligible for vote'}</span></div><button class="${selected?'teal':''}" ${disabled?'disabled':''} onclick="vote('${c.id}')">${selected?'Voted ✓':'Vote'}</button></div>`; }).join('')}</div>`; }
