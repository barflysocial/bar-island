...
function nav(){ const tabs=state.role==='player' ? [['home','Island'],['couples','Couples'],['challenge','Challenge'],['vote','Vote'],['recouple','Recouple']] : [['home','Island'],['couples','Couples'],['vote','Vote']]; return `<div class="nav"><div class="row between"><strong>🏝️ Bar Island</strong><span class="pill">Code ${esc(state.code)}</span></div><div class="tabs">${tabs.map(t=>`<button class="tab ${state.tab===t[0]?'active':''}" onclick="setTab('${t[0]}')">${t[1]}</button>`).join('')}</div></div>`; }
...
function myAnswer(){ const s=state.session, q=s?.currentQuestion; if(!q) return null; return (s.challengeAnswers||[]).find(a=>a.questionId===q.id && a.playerId===state.playerId) || null; }
function renderGame(){ const s=state.session; if(!s){ landing(); return; } syncClock(s.serverTime); app.innerHTML=nav()+`<section class="grid"><div>${mainPanel(s)}</div><div>${sidePanel(s)}</div></section>`; refreshClockBadges(); }
function mainPanel(s){ if(state.tab==='couples') return couplesPanel(s); if(state.tab==='challenge') return challengePanel(s); if(state.tab==='vote') return votePanel(s); if(state.tab==='recouple') return recouplePanel(s); return homePanel(s); }
function minglePanel(s){ if(!s.currentMingle) return ''; const c=myCouple(); const partnerNames=c?c.players.filter(p=>p.id!==state.playerId).map(p=>p.name).join(' + '):''; const starters=s.currentMingle.starters||[]; return `<div class="card phase-banner"><div class="row between"><h2>Couple Mingle Time</h2><span class="pill timer-pill">Talk Time <span data-countdown-target="${esc(s.currentMingle.endsAt)}">${countdownText(s.currentMingle.endsAt)}</span></span></div><p>${partnerNames?`Your partner: <b>${esc(partnerNames)}</b>`:'Meet your couple and get ready for the first challenge.'}</p><p class="muted">Prompts are optional during mingle time. Talk naturally, then use these only if you get stuck.</p><div class="grid">${starters.slice(0,6).map(x=>`<div class="profile">${esc(x)}</div>`).join('')}</div></div>`; }
function homePanel(s){ const m=me(); return `<div class="card phase-banner"><div class="row between"><span class="pill tag">Round ${s.round}/5</span><span class="pill">${esc(s.status)}</span>${countdownPill(s)}<span class="pill">Server <span data-server-clock>${liveClock()}</span></span></div><h1>${esc(s.title)}</h1><h2>${esc(s.currentPrompt)}</h2><p>${s.showtime?`Showtime: <b>${esc(s.showtime)}</b><br>`:''}${state.role==='player'&&m?`You are playing as <b>${esc(m.name)}</b>.`:'You are voting as audience.'}</p></div>
  ${s.currentMingle?minglePanel(s):''}
  ${s.currentQuestion && state.role==='player'?`<div class="card"><h2>Question Is Live</h2><p>${esc(s.currentQuestion.text)}</p><div class="row"><span class="pill timer-pill">Time Left <span data-countdown-target="${esc(s.currentQuestion.endsAt)}">${countdownText(s.currentQuestion.endsAt)}</span></span><button onclick="setTab('challenge')">Answer Now</button></div></div>`:''}
  <div class="card"><h2>Game Flow</h2><div class="grid"><div class="profile"><b>1. Couple Up</b><span class="muted">The host forms the first couples.</span></div><div class="profile"><b>2. Mingle</b><span class="muted">Couples get 5 minutes to talk before challenges.</span></div><div class="profile"><b>3. Timed Questions</b><span class="muted">Couples answer prompts from their phones.</span></div><div class="profile"><b>3. Vote</b><span class="muted">Players vote for other couples. Audience can vote for anyone.</span></div><div class="profile"><b>4. Recouple</b><span class="muted">Stay, switch, steal, or risk being single.</span></div><div class="profile"><b>5. Finale</b><span class="muted">Final vote crowns the champions.</span></div></div></div>${m?`<div class="card"><h2>Your Profile</h2>${profile(m)}</div>`:''}`; }
function sidePanel(s){ return `<div class="card"><h2>Island Status</h2><div class="row"><span class="pill">Players ${s.players.length}/${s.maxPlayers}</span><span class="pill">Couples ${s.couples.length}</span><span class="pill">Audience ${s.audienceCount}</span></div>${questionProgress(s)}${leaderboard(s)}</div><div class="card"><h2>Revealed Answers</h2>${answers(s)}</div>`; }
...
document.addEventListener('submit',async ev=>{
  if(ev.target.id==='answerForm'){
    ev.preventDefault(); const fd=new FormData(ev.target); const submitter=ev.submitter; const option=submitter?.name==='option' ? submitter.value : ''; const text=fd.get('text') || option;
