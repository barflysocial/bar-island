...
const app=$('#hostApp');
const state={ sessions:[], selected:null, timer:null, unlocked: localStorage.barIslandHostUnlocked === 'yes', hostPin: localStorage.barIslandHostPin || '', activeGameId: localStorage.barIslandActiveGameId || '', storage:'loading' };
...
function render(){
 app.innerHTML=`<div class="nav"><div class="row between"><strong>🏝️ Bar Island Host</strong><div class="row"><span class="pill">Storage: ${esc(state.storage)}</span><span class="pill">PIN unlocked</span><button class="secondary" onclick="lockHost()">Lock</button></div></div></div><section class="grid"><div>${sessionList()}</div><div>${hostPanel()}</div></section>`;
}
