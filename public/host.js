...
const app=$('#hostApp');
const state={ sessions:[], selected:null, timer:null, unlocked: sessionStorage.barIslandHostUnlocked === 'yes', hostPin: sessionStorage.barIslandHostPin || '' };
...
async function unlock(pin){
  ...
  state.hostPin=pin; state.unlocked=true; sessionStorage.barIslandHostPin=pin; sessionStorage.barIslandHostUnlocked='yes';
...
async function load(){
  ...
  if(state.selected){const found=state.sessions.find(s=>s.id===state.selected.id);state.selected=found||state.sessions[0]||null;}
  else state.selected=state.sessions[0]||null;
  render();
}
...
window.lockHost=()=>{sessionStorage.removeItem('barIslandHostUnlocked');sessionStorage.removeItem('barIslandHostPin');state.unlocked=false;state.hostPin='';clearInterval(state.timer);renderLocked();}
...
document.addEventListener('submit',async ev=>{
  ...
  if(ev.target.id==='createForm'){
    ...
    try{const data=await api('/api/sessions',{method:'POST',body:JSON.stringify(f)});state.selected=data.session;await load();toast('Scheduled game created.');}catch(e){toast(e.message)}
  }
});
