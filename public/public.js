...
function gameClock(s) {
  ...
  return fmt(Date.now() - new Date(s.gameStartedAt).getTime() - (s.gameClockPausedMs || 0));
}
  ...
  if (s.phase === 'lobby') {
    ...
    mainContent = `<div class="public-stage-layout">
      <div class="public-stage-main">
        ...
        <div class="cue">${winner ? `Winner: ${esc(coupleName(s, winner))}` : esc(cue(s))}</div>
      </div>
